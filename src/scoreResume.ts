#!/usr/bin/env node
/**
 * scoreResume.ts — Calculate ATS match score and identify missing keywords.
 *
 ...
 *
 * Usage:
 *   node dist/scoreResume.js <job-ad.txt> <tailored_resume.json> [--output score.json]
 *
...
import { extractKeywords } from './parseJobAd';
import type { JsonResume, ScoreReport, AtsResult } from './types';
 ...
 */
export function flattenResume(resume: JsonResume): string {
    const parts: string[] = [];
 ...
 */
export function checkAtsParsing(resume: JsonResume): AtsResult {
    const issues: string[] = [];
 ...
 */
export function calculateScore(jobAdText: string, resumeJson: JsonResume): ScoreReport {
...
function main(): void {
    ...
    if (args.length < 2 || args.includes('--help')) {
        console.log('Usage: node dist/scoreResume.js <job-ad.txt> <tailored_resume.json> [--output score.json]');
    ...
    const resumeJson: JsonResume = JSON.parse(fs.readFileSync(resumePath, 'utf-8'));
