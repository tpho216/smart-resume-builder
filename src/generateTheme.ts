#!/usr/bin/env node
/**
 * generateTheme.ts — Generate a custom JSON Resume theme (self-contained HTML
 ...
 *
 * Usage:
 *   node dist/generateTheme.js <structure.json> [--output themes/custom-theme/]
 *   node dist/generateTheme.js <structure.json> --preview resume.json
 *
...
import type { ContentStyle, StructureAnalysis, GeneratedTheme, SectionRenderer, LayoutHints } from './types';
...
export const SECTION_RENDERERS: Record<string, SectionRenderer> = {
    ...
    experience: {
        render: (style?: ContentStyle | null) => {
...
function generateCSS(layoutHints: Partial<LayoutHints> = {}): string {
    return `
 ...
 */
export function generateTheme(structure: StructureAnalysis): GeneratedTheme {
 ...
 */
export function previewWithTheme(themeIndexJs: string, resumeJson: JsonResumeLike): string {
    // Create a temporary module
    const m: { exports: { render?: (resume: JsonResumeLike) => string } } = { exports: {} };
    const fn = new Function('exports', 'module', themeIndexJs);
    fn(m.exports, m);
    return m.exports.render!(resumeJson);
}

// Type alias for the theme preview (doesn't need full JsonResume constraint)
type JsonResumeLike = Record<string, unknown>;
...
function main(): void {
    ...
    if (args.length === 0 || args.includes('--help')) {
        console.log('Usage: node dist/generateTheme.js <structure.json> [--output themes/custom/] [--preview resume.json]');
    ...
    const structure: StructureAnalysis = JSON.parse(fs.readFileSync(structurePath, 'utf-8'));
