#!/usr/bin/env node
/**
 * pipeline.ts — Run the full resume pipeline for a given job ad.
 *
 * Steps: parse → tailor → render → score
 *
 * Usage:
 *   node dist/pipeline.js <job-ad.txt> [--mode programmatic|llm]
 *   node dist/pipeline.js job_ads/senior_fullstack_engineer.txt --mode llm
 *   node dist/pipeline.js --all   (process all job ads in job_ads/)
 */
...
import type { JsonResume, PipelineResult } from './types';

const BASE_RESUME_PATH = path.join(__dirname, '..', 'base-resume.json');
const JOB_ADS_DIR = path.join(__dirname, '..', 'job_ads');
const OUTPUTS_DIR = path.join(__dirname, '..', 'outputs');

async function runPipeline(jobAdPath: string, mode: string = 'programmatic'): Promise<PipelineResult> {
...
async function main(): Promise<void> {
    ...
    if (args.length === 0 || args.includes('--help')) {
        console.log('Usage: node dist/pipeline.js <job-ad.txt> [--mode programmatic|llm]');
        console.log('       node dist/pipeline.js --all [--mode programmatic|llm]');
