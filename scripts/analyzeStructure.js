#!/usr/bin/env node
/**
 * analyzeStructure.js — Analyze an uploaded resume's section structure,
 * ordering, and hierarchy to produce a layout blueprint.
 *
 * Input: parsed resume JSON (from parseUploadedResume.js) or raw text file.
 * Output: A structure analysis JSON describing sections, their order, and
 *         content style (bullets, prose, grid, etc.).
 *
 * Usage:
 *   node scripts/analyzeStructure.js <parsed.json|resume.txt> [--output structure.json]
 */

const fs = require('fs');
const path = require('path');
const { extractParagraphs, detectHeadingCandidates } = require('./parseUploadedResume');
 ...
 * @param {string[]} lines - Content lines for a section
 * @returns {object} Style descriptor
 */
function analyzeContentStyle(lines) {
    ...
    let type = 'prose'; // default
    if (bulletRatio > 0.5) type = 'bullets';
    else if (shortLineCount / lines.length > 0.7 && avgLength < 40) type = 'list'; // skill tags, keyword list
 ...
 * @param {string[]} paragraphs - All paragraphs from the resume
 * @param {object[]} headingCandidates - From detectHeadingCandidates (optional)
 * @returns {object} Structure analysis
 */
function analyzeStructure(paragraphs, headingCandidates = null) {
    ...
    const hasSummary = sectionOrder.includes('summary') || (headerBlock && headerBlock.hasSummary);
    ...
    const skillsEarly = skillsPosition >= 0 && skillsPosition <= 2; // top of resume
...
function main() {
    ...
    if (args.length === 0 || args.includes('--help')) {
        console.log('Usage: node scripts/analyzeStructure.js <parsed.json|resume.txt> [--output structure.json]');
...
module.exports = { analyzeStructure, matchSection, analyzeContentStyle, SECTION_PATTERNS };
