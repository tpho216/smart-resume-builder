#!/usr/bin/env node
/**
 * generateDemo.js — Generate pre-built demo outputs for all dummy job ads
 * and Phase 2 sample resumes.
 * These outputs are committed to the repo for the GitHub Pages demo.
 *
 * Usage: node scripts/generateDemo.js
 */
...
const { renderBuiltInHtml, renderToPdf } = require('./renderResume');
...
async function main() {
    ...
    const phase2Index = [];
    const baseResume2 = JSON.parse(fs.readFileSync(BASE_RESUME_PATH, 'utf-8'));

    for (const file of sampleFiles) {
        const samplePath = path.join(SAMPLES_DIR, file);
        const sampleName = path.basename(file, '.txt');
        const outDir = path.join(OUTPUTS_DIR, `phase2_${sampleName}`);
        fs.mkdirSync(outDir, { recursive: true });

        console.log(`\nPhase 2 demo: ${sampleName}`);

        // Parse text (simulates PDF/DOCX upload)
        const text = fs.readFileSync(samplePath, 'utf-8');
        const paragraphs = extractParagraphs(text);
        const headingCandidates = detectHeadingCandidates(paragraphs).filter(h => h.isLikelyHeading);
        ...
        const html = previewWithTheme(theme.indexJs, baseResume2);
